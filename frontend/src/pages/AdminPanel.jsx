  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg-base)', padding: '32px 0' }}>
        <div className="container">
          ...
          <div className="admin-header">
            <div className="admin-title" style={{ fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: '2rem', letterSpacing: 3, textTransform: 'uppercase' }}>Panel Admin</div>
            <div className="admin-welcome" style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: 16 }}>Bienvenido, {admin?.username}</div>
            <div className="admin-actions">
              <button onClick={() => setShowSettings(!showSettings)} className="btn btn-ghost admin-btn">⚙ Ajustes</button>
              <button onClick={editingId ? cancel : () => setShowForm(!showForm)} className="btn btn-primary admin-btn">{showForm ? '✕ Cancelar' : '+ Añadir coche'}</button>
              <button onClick={logout} className="btn btn-ghost admin-btn">Salir</button>
            </div>
          </div>

          {showSettings && (
            <form onSubmit={submitAdmin} className="admin-form" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '32px 36px', marginBottom: 32, borderLeft: '4px solid var(--red)' }}>
          ...
          {showForm && (
            <form onSubmit={submit} className="admin-form" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '32px 36px', marginBottom: 32 }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
            ...
            {loading ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Cargando...</div>
              : cars.length === 0 ? <div style={{ padding: 60, textAlign: 'center', color: 'var(--text-muted)' }}><div style={{ fontSize: '3rem', marginBottom: 12 }}>🚗</div><div style={{ fontFamily: 'var(--font-display)', letterSpacing: 2 }}>SIN COCHES — Añade el primero</div></div>
                : cars.map((car, i) => (
                  <div key={car._id} className="admin-car-row" style={{ padding: '16px 24px', borderBottom: i < cars.length - 1 ? '1px solid var(--border)' : 'none' }}>
                    {/* Desktop layout */}
                    <div className="car-row-desktop">
                      <div className="car-thumb" style={{ width: 70, height: 50, borderRadius: 6, overflow: 'hidden', background: 'var(--bg-raised)', flexShrink: 0 }}>{car.fotos?.[0] ? <img src={getImgUrl(car.fotos[0])} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', fontSize: '1.5rem' }}>🚗</div>}</div>
                    ...
                    {/* Mobile layout */}
                    <div className="car-row-mobile">
                      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1rem', letterSpacing: 1, textTransform: 'uppercase', lineHeight: 1.2 }}>{car.marca} {car.modelo}</div>
                          <div style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: 2 }}>{car.año}</div>
                        </div>
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <div className="car-thumb-mobile" style={{ width: 80, height: 55, borderRadius: 6, overflow: 'hidden', background: 'var(--bg-raised)', marginBottom: 4 }}>{car.fotos?.[0] ? <img src={getImgUrl(car.fotos[0])} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', fontSize: '1.2rem' }}>🚗</div>}</div>
                          <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{formatNum(car.kilometros)} km</div>
                          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1rem', color: 'var(--red)' }}>{formatNum(car.precio)}€</div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, marginTop: 10, alignItems: 'center' }}>
                        <span className={`badge ${car.disponible ? 'badge-hibrido' : 'badge-gasolina'}`}>{car.disponible ? 'Disponible' : 'Vendido'}</span>
                        {car.destacado && <span className="badge badge-gasolina">★</span>}
                        <button onClick={() => edit(car)} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-steel)', color: 'white', borderRadius: 4, padding: '5px 10px', fontSize: '0.75rem', fontFamily: 'var(--font-display)', letterSpacing: 1, textTransform: 'uppercase' }}>Editar</button>
                        <button onClick={() => remove(car._id, `${car.marca} ${car.modelo}`)} style={{ background: 'rgba(214,40,40,0.1)', border: '1px solid rgba(214,40,40,0.3)', color: 'var(--red)', borderRadius: 4, padding: '5px 10px', fontSize: '0.75rem', fontFamily: 'var(--font-display)', letterSpacing: 1, textTransform: 'uppercase' }}>Eliminar</button>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <style>{`
        ...
        .admin-btn { font-size: 0.85rem; }
        .car-row-desktop {
          display: flex;
          align-items: center;
          gap: 16px;
        }
        .car-row-mobile { display: none; }
        ...
        @media(max-width:640px) {
          ...
          .admin-welcome { display: none !important; }
          .admin-title { font-size: 1.4rem !important; letter-spacing: 2px !important; margin-bottom: 12px !important; }
          .admin-actions {
          ...
          .admin-form { padding: 20px 16px !important; }
          .form-grid-2 { grid-template-columns: 1fr !important; }

          .car-row-desktop { display: none !important; }
          .car-row-mobile { display: block !important; }
          .admin-car-row { padding: 14px 16px !important; }
        }
      `}</style>
    </>
  );
